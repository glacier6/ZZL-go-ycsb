...
./bin/go-ycsb load badger -P workloads/workloada -P zzl_badger.properties -p threadcount=16
if [ $? -ne 0 ]; then
    echo "❌ Load 阶段报错，停止压测！"
    exit 1
fi
...
./bin/go-ycsb run badger -P workloads/workloada -P zzl_badger.properties -p threadcount=16
