...
run_single_test() {
    ...
    if [ "$template_exists" = "yes" ]; then
        ...
        sshpass -p "$remote_password" ssh -o StrictHostKeyChecking=no -o ServerAliveInterval=60 -o ServerAliveCountMax=10 ${remote_user}@${REMOTE_IP} \
            "cd ${REMOTE_WORKSPACE} && ./bin/go-ycsb load badger -P workloads/workloadav -P zzl_badger.properties -p threadcount=${THREADS} -p recordcount=${RC} -p stabilization_time=${STABILIZE_TIMES[0]}" >> ${LOG_FILE} 2>&1
        if [ $? -ne 0 ]; then
    ...
    sshpass -p "$remote_password" ssh -o StrictHostKeyChecking=no -o ServerAliveInterval=60 -o ServerAliveCountMax=10 ${remote_user}@${REMOTE_IP} \
        "cd ${REMOTE_WORKSPACE} && ./bin/go-ycsb run badger -P workloads/workloadav -P zzl_badger.properties -p threadcount=${THREADS} -p recordcount=${RC} -p operationcount=${OPCOUNT} -p stabilization_time=${STABILIZE_TIMES[1]}" >> ${LOG_FILE} 2>&1
...
for RC in "${RECORD_COUNTS[@]}"; do
    ...
    for TC in "${THREAD_COUNTS[@]}"; do
        ...
        for (( i=1; i<=ROUNDS; i++ )); do
            ...
            # 跑魔改版 (heatLSM) Badger
            run_single_test "heatLSM_Badger_NL98M_V" ${PRIZZL_BADGER_PATH} $i $TC $RC

            # 跑原版 Badger
            run_single_test "Normal_Badger_V" ${NORMAL_BADGER_PATH} $i $TC $RC
        done
    done
done
